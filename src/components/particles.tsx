...
export function Particles({
  ...
  const drawCircle = (circle: Circle, update = false) => {
    if (context.current) {
      ...
      context.current.fillStyle = `rgba(138, 92, 245, ${alpha})`;
  ...
  const animate = () => {
    ...
    circles.current.forEach((circle: Circle, i: number) => {
      ...
      const edge = [
        ...
        containerSize.current.w - circle.x - circle.translateX,
        circle.y + circle.translateY,
        containerSize.current.h - circle.y - circle.translateY,
      ];
  ...
  return (
    <div className={cn('absolute inset-0 z-0', className)} ref={canvasContainerRef}>
